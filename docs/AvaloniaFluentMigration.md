...
| Mutually exclusive geometry | Windows owns its frame, clipping, shadow, and caption as one composition system. | Independent corner switches can represent contradictory states and make one option silently override another. | Model corner shape as one enum: Windows 10 square, Windows 11 rounded, or macOS rounded. Model control style separately as Windows or macOS. Persist the two selections independently. |
...
| Native macOS controls | Not applicable to the Windows source application. | `WindowDecorations=Full` couples traffic lights to AppKit's title bar and geometry. Instead, obtain standard close, minimize, and zoom buttons through `+[NSWindow standardWindowButton:forStyleMask:]`, attach them above the Avalonia content, and target the window's standard actions. This preserves native behavior while keeping controls independent from corner style. |
